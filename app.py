...
Upgraded rules, live color-coded UI, speed/ETA tracker, strict custom-domain checking.
"""

import streamlit as st
import re, io, smtplib, time, random, string
import pandas as pd
...
# ── CONSTANTS ─────────────────────────────────────────────────────────────────────────
EMAIL_REGEX = re.compile(r"[a-zA-Z0-9._%+\-]+@[a-zA-Z009.\-]+\.[a-zA-Z]{2,}", re.IGNORECASE)
...
def fetch_disposable_domains():
    try:
        ...
        if r.status_code == 200:
            online = {line.strip().lower() for line in r.text.splitlines() if line.strip() and not line.startswith('#')}
            return online.union(set(_DISPOSABLE_FALLBACK))
    except Exception:
        pass
...
def _val_mailbox(email, mx_records):
    try:
        ...
        with smtplib.SMTP(mx, timeout=8) as s:
            s.helo("validator.pro"); s.mail("test@example.com")
            code, _ = s.rcpt(email)
            return code == 250
    except: return False
...
def _deliverability(syntax, mx_ok, mailbox_ok, disposable, free, catch_all, spf_ok):
    ...
    if mailbox_ok:
        ...
        # SMTP check failed/timed out
        if free:
            # Gmail/Outlook block SMTP. Trust valid syntax + MX for free providers.
            return "Deliverable", "Free provider"
        # CUSTOM DOMAINS: If we can't verify the mailbox, it almost certainly doesn't exist.
        if catch_all:  return "Risky", "Catch-all, mailbox unknown"
        if not spf_ok: return "Not Deliverable", "No SPF - spam risk"
        return "Not Deliverable", "Mailbox does not exist"
...
def validate_email_full(email):
    ...
    ca    = _val_catch_all(domain, mx_h) if (mx_ok and DNS_AVAILABLE and not free and not disp_) else False
...
def build_xlsx(results, original_columns):
    ...
    val_cols = [("Validated Email",32), ("Status",16), ("Score",8), ("Tier",9),
                 ("Reason",22), ("SPF",6), ("DMARC",7), ("Catch-all",10),
                 ("Fallback?",10), ("Emails Checked",14)]
    val_offset = len(original_columns)
    for ci, (n, w) in enumerate(val_cols, val_offset + 1): _hdr(ws, 1, ci, n, w=w)
    ...
    for ri, row in enumerate(results, 2):
        ...
        sf_ = SF.get(st_); w_col = "FFFFFF" if sf_ else "111111"
    ...
    for ci, (n, w) in enumerate([("#",6),("Domain",22),("Original Best",32),("Email Checked",32),
                                  ("Status",16),("Reason",22),("Result",14)], 1): _hdr(ws2, 1, ci, n, w)
    r2 = 2
    for ri, row in enumerate(results, 1):
        ...
        for li, (ce, cs, cr) in enumerate(vl):
            ...
            sf2 = SF.get(cs); s_col = "FFFFFF" if sf2 else "111111"
    ...
    _stats_sheet(wb, "Stats", [
        ("Total rows in CSV", nt, "total"), ("Rows with no email (retained)", n_empty, "none"),
        ("Rows validated", n_val_rows, "total"), ("Deliverable", nd, "deliverable"),
        ("Risky", nri, "risky"), ("Not Deliverable", nb, "fail"),
        ("Fallback emails used", nfb, "fallback"), ("Total emails checked", tc, "total"),
        ("Extra checks (fallbacks)", saved, "fallback"), ("Avg checks per valid row", ac, "avg"),
...
#  STREAMLIT APP
# ══════════════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="CSV Email Validator", page_icon="✅", layout="wide", initial_sidebar_state="expanded")
ACCENT = "#16a34a"
...
[data-testid="stSidebar"] .stDownloadButton > button {{ background:{ACCENT} !important; border:none !important; color:#fff !important; border-radius:8px !important; font-size:12px !important; font-weight:700 !important; width:100% !important; }}
...
[data-testid="stMetricLabel"] p {{ font-size:9.5px !important; font-weight:700 !important; color:#c0bfbb !important; text-transform:uppercase !important; letter-spacing:.6px !important; }}
[data-testid="metricValue"] {{ font-size:22px !important; font-weight:800 !important; color:#111 !important; letter-spacing:-.7px !important; }}
...
with st.sidebar:
    ...
    if res:
        ...
        st.download_button("Export .xlsx", xlsx, f"validated_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="sx_xlsx", use_container_width=True)
        st.divider()
        ...
        saved = tc - nv
        st.markdown(f'<div style="font-size:11px;color:#666;line-height:2.2">Total rows: <strong style="color:#fff">{len(res)}</strong><br>Checked: <strong style="color:#fff">{nv}</strong><br>Deliverable: <strong style="color:#4ade80">{nd}</strong><br>Fallbacks: <strong style="color:#22d3ee">{nf}</strong><br>Checks saved: <strong style="color:#22d3ee">{saved}</strong></div>', unsafe_allow_html=True)
    st.divider()
    st.markdown('<div style="font-size:9px;color:#333;line-height:1.8">Upload CSV -> pick 2 columns<br>Best Email -> validated first<br>All Emails -> fallback pool<br>Empty rows kept in XLSX<br>Stops on first Deliverable<br><br><strong>Strict custom rules:</strong><br>Dead custom .coms marked accurately</br>Gmail/Outlook trusted if valid</div>', unsafe_allow_html=True)

# ── Session State ─────────────────────────────────────────────────────────────
for k, v in {"cv_results":[],"cv_running":False,"cv_idx":0,"cv_log":[],"cv_df":None,
             "cv_queue":[],"cv_original_cols":[],"cv_start_time":0.0}.items():
    if k not in st.session_state: st.session_state[k] = v

# ── Header ────────────────────────────────────────────────────────────────────
st.markdown(f"""<div class="mh-ph"><div class="mh-pi">✅</div><div><div class="mh-pt">CSV Email Validator</div><div class="mh-ps">upload CSV · pick columns · early-stop on first Deliverable · keeps all original data</div></div></div>""", unsafe_allow_html=True)
...
if df is not None:
    ...
    st.markdown(f'<div class="cp"><div class="cp-l">Best Email column</div><div class="cp-v">' + "<br>".join(str(v)[:60] for v in df[best_col].head(3).values) + '</div></div>', unsafe_allow_html=True)
    if all_col != "— None —":
        st.markdown(f'<div class="cp"><div class="cp-l">All Emails column</div><div class="cp-v">' + "<br>".join(str(v)[:80] for v in df[all_col].head(3).values) + '</div></div>', unsafe_allow_html=True)
    ...
    for i, row in df.iterrows():
        ...
        if not has_emails: dom = f"row_{i+1}"
        elif dom_col != "— Auto —" and pd.notna(row.get(dom_col)): dom = str(row[dom_col]).strip()
        elif be: dom = be.split("@")[-1]
        else: dom = ae[0].split("@")[-1]
        queue.append({"row_idx": i+1, "domain": dom, "original_email": be, "all_emails": ae, "has_emails": has_emails, "original_row_data": row.to_dict()})
    ...
    with vc1:
        ...
        if not running:
            if st.button(f"Validate {nv} row(s)", type="primary", use_container_width=True, disabled=not nv, key="cv_go"):
                st.session_state.cv_results = []; st.session_state.cv_idx = 0; st.session_state.cv_log = []
                st.session_state.cv_running = True; st.session_state.cv_queue = queue
    ...
    # ── LIVE PROGRESS & LOG ───────────────────────────────────────────────────
    res = st.session_state.cv_results; cq = st.session_state.get("cv_queue", queue)
    ci = st.session_state.cv_idx; tot = len(cq)
    if running and tot > 0:
        ...
        remaining = nv - valid_done
        eta_sec = remaining / speed if speed > 0 else 0
        eta_str = str(timedelta(seconds=int(eta_sec))) if remaining > 0 else "0s"
        ...
        st.markdown(f'<div style="font-size:12px;font-weight:700;color:#111;margin:6px 0 2px">Validating {valid_done}/{nv} — <code style="color:{ACCENT}">{ce[:40]}</code></div><div class="vp"><div class="vf" style="width:{pct}%"></div></div><div style="font-size:20px;font-weight:800;color:{ACCENT};text-align:right;margin-top:-4px">{pct}%</div>', unsafe_allow_html=True)

        st.markdown(f'<div style="font-size:11px;color:#666;display:flex;gap:20px;flex-wrap:wrap;margin:8px 0"><span>Deliverable: <strong style="color:#16a34a">{nd}</strong></span><span>Risky: <strong style="color:#d97706">{nri}</strong></span><span>Failed: <strong style="color:#dc2626">{nb}</strong></span><span>Speed: <strong>{speed:.1f}/s</strong></span><span>ETA: <strong>{eta_str}</strong></span></div>', unsafe_allow_html=True)
    ...
    if res:
        ...
        for r in res:
            ...
            if not r.get("has_emails"):
                rows.append({"#":r["row_idx"],"Domain":r["domain"],"Original":"(empty)","Validated":"—", "Status":"Skipped","Tier":"—","Score":"—","Reason":"—","SPF":"—","DMARC":"—","CA":"—","Pool":0,"Checked":0})
                continue
        ...
        def color_status(row):
            styles = [""] * len(row)
            if row["Status"] == "Deliverable": styles[4] = "background-color: #d4edda; color: #155724"
            elif row["Status"] == "Risky": styles[4] = "background-color: #fff3cd; color: #856404"
            elif row["Status"] == "Not Deliverable": styles[4] = "background-color: #f8d7da; color: #721c24"
            elif row["Status"] == "Skipped": styles[4] = "background-color: #f9fafb; color: #999999"
            return styles
        
        styled_dr = dr.style.apply(color_status, axis=1)
        st.caption(f'**{len(dr)}** of {len(res)}  |  (fallback) = switched email  |  Pool = fallback size  |  Checked = emails validated before stop')
        st.dataframe(styled_dr, use_container_width=True, hide_index=True,
    ...
    if st.session_state.cv_running:
        ...
        if idx >= tot:
            ...
            if not item.get("has_emails"):
                st.session_state.cv_results.append({"row_idx":rn,"domain":dom,"original_email":"","validated_email":"","all_emails":[],"val":None,"was_fallback":False,"confidence":None,"val_log":[], "original_row_data": orig_data, "has_emails": False})
                next_idx = idx + 1
                while next_idx < tot and not q[next_idx].get("has_emails"):
                    n_item = q[next_idx]
                    st.session_state.cv_results.append({"row_idx": n_item["row_idx"], "domain": n_item["domain"], "original_email":"","validated_email":"","all_emails":[],"val":None,"was_fallback":False,"confidence":None,"val_log":[], "original_row_data": n_item.get("original_row_data", {}), "has_emails": False})
                    next_idx += 1
                ...
                st.session_state.cv_results.append({"row_idx":rn,"domain":dom,"original_email":best,"validated_email":val_em,"all_emails":ae,"val":val_res,"was_fallback":was_fb,"confidence":cf,"val_log":vlog, "original_row_data": orig_data, "has_emails": True})
...
if df is None and not st.session_state.cv_results:
    ...
    <div style="text-align:center;padding:60px 0">
        ...
        <div style="font-size:12.5px;color:#aaa;line-height:2;max-width:420px;margin:0 auto">
            ...
            (semicolon, comma, pipe, or <code> * </code> separated) as fallback pool.<br><br>
